...
# Importar módulos mejorados de clasificación y deduplicación
sys.path.append(project_root)  # Asegurar que podemos importar desde el directorio raíz
try:
    ...
    logger.warning(f"No se pudieron cargar los módulos mejorados: {e}. Se usará el sistema estándar.")
    ENHANCED_MODULES_AVAILABLE = False

# Importar el deduplicador optimizado
try:
    from optimized_deduplicator import OptimizedDeduplicator
    OPTIMIZED_DEDUP_AVAILABLE = True
    logger.info("Deduplicador optimizado cargado correctamente")
except ImportError as e:
    logger.warning(f"No se pudo cargar el deduplicador optimizado: {e}. Se usará el sistema estándar.")
...
def run_pipeline(custom_date_str=None):
    ...
    try:
        ...
        # Inicializar el deduplicador optimizado si está disponible
        if OPTIMIZED_DEDUP_AVAILABLE:
            deduplicator = OptimizedDeduplicator(paths['cache_dir'])
            logger.info("Deduplicador optimizado inicializado correctamente")
        else:
            logger.warning("Usando sistema de deduplicación estándar")
    ...
    try:
        ...
        if not links_to_process:
             ...
             save_stats(stats, paths['processing_stats_json'])
             logger.info(f"Estadísticas guardadas en {paths['processing_stats_json']}")
             return  # Salir si no hay URLs nuevas
             
        # --- 4.1 Detección de duplicados basada en contenido ---
        duplicates_removed = 0
        if ENHANCED_MODULES_AVAILABLE:
            try:
                # Inicializar deduplicador
                logger.info("--- Paso 2.1: Analizando duplicados basados en contenido ---")
                deduplicator = ContentDeduplicator(cache_dir=os.path.join(paths['cache_dir'], 'dedup'))
                
                # Guardar URLs antes de filtrar
                pre_dedup_count = len(links_to_process)
                
                # Extraer URLs de la lista de diccionarios
                url_list = [item['URL'] for item in links_to_process if 'URL' in item]
                
                # Filtrar duplicados
                unique_urls = deduplicator.filter_duplicates(url_list)
                
                # Actualizar lista de links manteniendo solo URLs únicas
                links_to_process = [item for item in links_to_process 
                                   if 'URL' in item and item['URL'] in unique_urls]
                
                # Actualizar contador de duplicados
                duplicates_removed = pre_dedup_count - len(links_to_process)
                
                # Guardar análisis de duplicados
                duplicates_path = os.path.join(
                    project_root, 'output', f'duplicates_analysis_{today_date_for_filename}.json'
                )
                duplicate_stats = {
                    "timestamp": datetime.now().isoformat(),
                    "original_count": pre_dedup_count,
                    "unique_count": len(links_to_process),
                    "duplicates_removed": duplicates_removed,
                    "percentage_reduced": round(duplicates_removed / pre_dedup_count * 100, 2) if pre_dedup_count > 0 else 0
                }
                save_to_json(duplicate_stats, duplicates_path)
                
                logger.info(f"Análisis de duplicados completado: {duplicates_removed} URLs duplicadas filtradas")
                logger.info(f"URLs después de filtrar duplicados: {len(links_to_process)}")
                logger.info(f"Estadísticas de duplicados guardadas en {duplicates_path}")
                
            except Exception as e:
                logger.warning(f"Error en análisis de duplicados: {e}. Continuando sin filtrar duplicados.")
                # No realizar filtrado si hay error
        ...
        if ENHANCED_MODULES_AVAILABLE:
            try:
                # Usar el clasificador avanzado primero
                url_classifier = URLClassifier()
                detailed_classification = url_classifier.classify_urls(links_to_process)
                
                # Guardar clasificación detallada para referencia
                detailed_classification_path = os.path.join(
                    project_root, 'output', f'detailed_classification_{today_date_for_filename}.json'
                )
                save_to_json(detailed_classification, detailed_classification_path)
                logger.info(f"Clasificación detallada guardada en: {detailed_classification_path}")
                
                # Convertir al formato esperado por el sistema
                categories = url_classifier.get_legacy_format(detailed_classification)
                logger.info("URLs clasificadas con sistema mejorado")
                
                # Estadísticas más detalladas
                urls_summary = {
                    "total_found": len(all_links),
                    "filtered_by_history": len(all_links) - len(links_to_process),
                    "filtered_duplicates": duplicates_removed,
                    "to_process": len(links_to_process),
                    "categories": {
                        "html": {
                            "total": len(categories.get('html', [])),
                            "news": len(detailed_classification.get('html', {}).get('news', [])),
                            "government": len(detailed_classification.get('html', {}).get('government', [])),
                            "educational": len(detailed_classification.get('html', {}).get('educational', [])),
                            "documents": len(detailed_classification.get('html', {}).get('documents', [])),
                            "other_html": len(detailed_classification.get('html', {}).get('other_html', []))
                        },
                        "social": {
                            "total": len(categories.get('social', [])),
                            "facebook": len(detailed_classification.get('social', {}).get('facebook', [])),
                            "youtube": len(detailed_classification.get('social', {}).get('youtube', [])),
                            "twitter": len(detailed_classification.get('social', {}).get('twitter', [])),
                            "other_social": len(detailed_classification.get('social', {}).get('other_social', []))
                        },
                        "images": len(categories.get('images', [])),
                        "audio": len(detailed_classification.get('audio', [])),
                        "other": len(categories.get('other', []))
                    }
                }
                
                # Extraer URLs por categoría y subcategoría para procesamiento específico
                if 'social' in detailed_classification and 'facebook' in detailed_classification['social']:
                    facebook_urls = [item['URL'] for item in detailed_classification['social']['facebook']]
                    logger.info(f"URLs específicas de Facebook identificadas: {len(facebook_urls)}")
                
                if 'social' in detailed_classification and 'youtube' in detailed_classification['social']:
                    youtube_urls = [item['URL'] for item in detailed_classification['social']['youtube']]
                    logger.info(f"URLs específicas de YouTube identificadas: {len(youtube_urls)}")
                
            except Exception as e:
                logger.warning(f"Error en clasificación avanzada: {e}. Usando clasificación estándar.")
                # Fallback al clasificador original si hay error
                ...
                urls_summary = {
                    ...
                    "filtered_duplicates": duplicates_removed,
                    "to_process": len(links_to_process),
                    "html": len(categories.get('html', [])),
                    "images": len(categories.get('images', [])),
                    "social": len(categories.get('social', [])),
                    "other": len(categories.get('other', []))
                }
        else:
            # Usar el clasificador estándar si no están disponibles los módulos mejorados
            categories = classify_urls(links_to_process)
            
            # El resumen de URLs se guarda en la sección de clasificación mejorada
